...
export default function Services() {
    ...
    const TABS = useMemo(() => [
        ...
        ...priceCategories.map(c => ({
            id: c.id,
            label: c.label,
            hasSteamIron: c.hasSteamIron
        })),
    ], [priceCategories])

    const allItems = useMemo(() => priceCategories.flatMap(cat =>
        cat.items.map(item => ({
            ...item,
            _cat: cat.label,
        ...
        data.forEach(item => {
            ...
            if (!grouped[key]) {
                grouped[key] = {
                    ...
                    hasSteamIron: item.hasSteamIron === true || item.hasSteamIron === "TRUE",
                    items: []
                }
            }
            ...
            grouped[key].items.push({
                ...
                steam: item.steam ? Number(item.steam) : null
            })
        })
    ...
    useEffect(() => {
        const fetchPrices = async () => {
            try {
                const res = await fetch("https://script.google.com/macros/s/AKfycbzEVOoey-GpT71c00alxyS_WqdWkGQtzTf5QhV4UXEzZEGKDJmIwkIk0-LvEDlqE731/exec")
                const data = await res.json()

                const formatted = transformData(data)
                setPriceCategories(formatted)
            } catch (err) {
